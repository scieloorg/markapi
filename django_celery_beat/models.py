...
def crontab_schedule_celery_timezone():
    """
    Return the timezone string from the Django settings `CELERY_TIMEZONE` variable.

    If it's not defined or is not a valid timezone, return `"UTC"` instead.
    """
    try:
        celery_timezone = getattr(settings, f"{getattr(settings, 'CELERY_APP_NAMESPACE', 'CELERY')}_TIMEZONE", "UTC")
    except AttributeError:
        # Default to UTC if the namespace or setting is not defined
        return "UTC"

    # Validate against the list of available timezones
    if celery_timezone in all_timezones:
...
class SolarSchedule(models.Model):
    ...
    def __str__(self):
        return "{0} ({1}, {2})".format(
            self.event, self.latitude, self.longitude
        )
